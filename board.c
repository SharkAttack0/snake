...
// void update_snake_full_board(enum board_state board[BOARD_SIZE_X][BOARD_SIZE_Y], struct coordinates *snake, int snake_length) {
//     board[snake[0].x][snake[0].y] = SNAKE;
//     board[snake[snake_length].x][snake[snake_length].y] = EMPTY;
//     return;
// }
...
void update_board(enum board_state board[BOARD_SIZE_X][BOARD_SIZE_Y], struct coordinates* snake, int snake_length, struct coordinates consumable) {
    //update_snake_full_board(board, snake, snake_length);
    // way 2:
    for(int i = 0; i < BOARD_SIZE_X; i++){
        for(int j = 0; j < BOARD_SIZE_Y; j++) {
            if (board[i][j] == SNAKE || board[i][j] == CONSUMABLE) {
                board[i][j] = EMPTY;
            } 
        }
    }
